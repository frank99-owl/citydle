...
**Citydle**（城脉）是一个每日地图推理小游戏。每天出一道题：给你一座真实城市的路网骨架——以羊皮纸风格的制图轮廓呈现——用 **6 条线索**，从 6 个候选城市里认出它是哪儿。用的线索越少，得分越高。
...
| 状态 | 说明 |
|------|------|
| 城市库 | **100 座高辨识度世界城市**——每日一题一轮 100 天不重复 |
| 选城标准 | 有海岸线、有河流、路网形态独特（网格状、放射状、不规则老城）；排除「哪座城市都长一样」的纯住宅郊区 |
| 生成管线 | `fetch-cities.mjs`（Overpass，人工划定代表区）→ `validate-cities.mjs`（机器校验门，0 错误）→ `compute-morphology.mjs`（网格度+水系分类，驱动干扰项相似度） |
