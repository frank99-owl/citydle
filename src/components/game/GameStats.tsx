...
function getEncouragement(rate: number, lang: Language): string | null {
  if (rate >= 0.9) return lang === 'zh' ? '只差几步即可完美通关！' : 'Just a few more to perfect completion!';
  if (rate >= 0.75) return lang === 'zh' ? '即将征服此图！' : 'Almost conquered this map!';
  if (rate >= 0.5) return lang === 'zh' ? '已过半程，继续加油！' : 'Halfway there, keep going!';
  return null;
}
...
export const GameStats = memo(function GameStats({ lang, mapName, guessedCount, totalStreets }: GameStatsProps) {
  ...
  return (
    <div style={{
      ...
      {/* Progress bar */}
      <div style={{
        width: '100%', height: '10px', background: '#d9d0bc',
        borderRadius: '5px', overflow: 'hidden', margin: '0.5rem 0',
      ...
      {remaining > 0 && (
        <div style={{
          textAlign: 'center', fontSize: '0.78rem', color: '#7a6b55',
          marginBottom: '0.3rem', fontFamily: 'var(--font-cinzel)',
        }}>
      ...
      {encouragement && (
        <div style={{
          textAlign: 'center', marginTop: '0.5rem', padding: '0.35rem 0.6rem',
          background: '#fdf8eb', border: '1px dashed #c5a059',
          borderRadius: '2px', fontSize: '0.8rem', color: '#8a6d3b',
