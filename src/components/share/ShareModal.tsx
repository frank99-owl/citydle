...
export function ShareModal({
  ...
  const { downloadImage, copyShareLink, shareToTwitter, nativeShare } = useShare();
  ...
  const handleDownload = useCallback(() => {
    if (shareBlob) {
      downloadImage(shareBlob, `street-cartographer-${cityName.replace(/\s+/g, '-').toLowerCase()}.png`);
    }
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            background: 'none', border: 'none', color: '#4e3629',
        ...
        <div style={{
          ...
          {generating ? (
            <div style={{ padding: '2rem', textAlign: 'center', color: '#4e3629', fontStyle: 'italic' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          ...
          >
            {copied
              ? (isZh(lang) ? '✓ 已复制' : '✓ Copied')
              : (isZh(lang) ? '🔗 复制链接' : '🔗 Copy Link')
            }
          </button>
