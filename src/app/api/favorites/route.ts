...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!name || !bounds) {
      return NextResponse.json({ error: 'Missing name or bounds' }, { status: 400 });
    }

    const stmt = db.prepare(
      'INSERT INTO favorite_maps (user_id, name, city_name, bounds) VALUES (1, ?, ?, ?)'
    );
    const result = stmt.run(name, cityName ?? null, JSON.stringify(bounds)) as { lastInsertRowid: number };
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    db.prepare('DELETE FROM favorite_maps WHERE id = ? AND user_id = 1').run(id);
