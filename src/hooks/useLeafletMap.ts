...
export function useLeafletMap({ toMapLatLng }: UseLeafletMapOptions) {
  ...
  // Sync tile layer
  const syncTileLayer = useCallback(
  ...
  // Draw street polylines (incremental: only rebuild layers when street set changes)
  const drawStreets = useCallback(
    (streets: Street[], showResult: boolean) => {
      const map = mapRef.current;
      if (!map) return;

      const getStyle = (guessed: boolean) => {
        if (guessed) return { color: "#3a5f43", opacity: 0.8 };
        if (showResult) return { color: "#8a3324", opacity: 0.8 };
        return { color: "#8a3324", opacity: 0 };
      };

      // Check if existing layers match the incoming streets (same key set)
      const existingKeys = Object.keys(geojsonLayersRef.current);
      const incomingKeys = streets
        .map((s) => s.name.toLowerCase().trim())
        .filter((k) => {
          const street = streets.find((s) => s.name.toLowerCase().trim() === k);
          return street?.geometry && street.geometry.length > 0;
        });
      const isSameSet =
        existingKeys.length === incomingKeys.length &&
        incomingKeys.every((k) => k in geojsonLayersRef.current);

      if (isSameSet) {
        // Incremental update: just refresh styles (handles guessed-state changes)
        streets.forEach((street) => {
          const key = street.name.toLowerCase().trim();
          const layer = geojsonLayersRef.current[key];
          if (layer) {
            layer.setStyle(getStyle(street.guessed));
          }
        });
        return;
      }

      // Full rebuild: new game or different street set
      import("leaflet").then((L) => {
        ...
        streets.forEach((street) => {
          if (street.geometry && street.geometry.length > 0) {
            const mappedGeom = street.geometry.map(([lat, lng]) =>
              toMapLatLng(lat, lng),
            );
            const style = getStyle(street.guessed);
            ...
            geojsonLayersRef.current[street.name.toLowerCase().trim()] =
              polyline;
          }
        });
      });
  ...
  const setupDrawing = useCallback(
    (
      enabled: boolean,
  ...
  const shiftMapCenter = useCallback(
    (
      oldProvider: string,
      newProvider: string,
      convertFn: (
