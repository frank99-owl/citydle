...
* **CartoDB 浅色 & 深色**：默认防作弊底图（无路名标签）。
* **OpenStreetMap**：提供包含完整标注的传统地图。
* **高德地图 (Amap)**：提供国内高速瓦片服务。由于高德地图采用火星坐标系 (GCJ-02)，而系统底层及 API 始终使用标准的 GPS 地球坐标系 (WGS-84)，系统在 Leaflet 渲染边界层实现了动态无缝的双向纠偏转换（仅在高德图层被选中时生效）。
* **中途切换重绘**：在游戏中途切换地图源时，系统会自动重绘并对齐当前视图中心、拉框矩形、街道折线以及闪烁的提示图层。
